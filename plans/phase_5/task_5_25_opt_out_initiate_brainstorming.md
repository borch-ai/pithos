...
### Manual Verification
1. Run `./bin/pithos initiate --theme "A pessimistic turtle" --output books/turtle_book` and verify that the manifest contains generated `book_properties.style` and `character_profile` fields.
2. Run `./bin/pithos initiate --theme "A pessimistic turtle" --output books/turtle_book_no_brain --no-brainstorm` and verify that the manifest contains empty `book_properties.style` and `character_profile` fields.
