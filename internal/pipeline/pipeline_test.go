...
func TestBrew_CharacterInvariantInjection_WebmVideoSeed(t *testing.T) {
	tmpDir := t.TempDir()

	// 1. Write a dummy source video to be copied
	dummySourceImage := filepath.Join(tmpDir, "source.WEBM")
	if err := os.WriteFile(dummySourceImage, []byte("fake-video-bytes"), 0600); err != nil {
		t.Fatalf("failed to write source image: %v", err)
	}
	...
	optsInit := InitiateOptions{
		OutputDir:       tmpDir,
		Theme:           "Invariant Test Theme Webm",
		TargetPageCount: 2,
	}
	mInit, err := Initiate(optsInit)
	if err != nil {
	...
	err = Brew(ctx, opts)
	if err == nil {
		t.Fatal("expected Brew to fail due to invalid webm format during ffmpeg extraction, got nil")
	}
	if !strings.Contains(err.Error(), "failed to extract static frame from character seed video") {
		t.Errorf("unexpected error: %v", err)
	}
